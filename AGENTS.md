...
```bash
python -m py_compile jupyter_ai_tutor/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'jupyter_ai_tutor:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_ai_tutor:open-panel';
  export const refreshData = 'jupyter_ai_tutor:refresh-data';
}
...
- **Output**: Creates files in `lib/` and `jupyter_ai_tutor/labextension/`
...
- Ask user to search for the extension ID (`jupyter_ai_tutor`) to see if it loaded
...
ls -la jupyter_ai_tutor/labextension/  # Should contain bundled extension
```
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_ai_tutor:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_ai_tutor:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-ai-tutor-ClassName`
